...
use Lv\RpcBundle\Server\Exceptions\InvalidMappingException;
...
class Mapper implements ContainerAwareInterface
{
    ...
    public function loadClassMetadata($class)
    {
        ...
        if (class_exists($class)) {
            ...
            if ($method = $this->reader->getClassAnnotation($reflectionClass, 'Lv\RpcBundle\Mapping\Method')) {
                ...
                // Cache
                $meta['cache'] = $this->reader->getClassAnnotation($reflectionClass, 'Lv\RpcBundle\Mapping\Cache');

                // Roles
                $meta['roles'] = $this->reader->getClassAnnotation($reflectionClass, 'Lv\RpcBundle\Mapping\Roles');

                // Deprecated
                // Env
                // $meta['env'] = $this->reader->getClassAnnotation($reflectionClass, 'Lv\RpcBundle\Mapping\Env');
                ...
                foreach ($reflectionClass->getMethods() as $reflectionMethod) {
                    ...
                    if ($paramMeta = $this->reader->getMethodAnnotation($reflectionMethod, 'Lv\RpcBundle\Mapping\Execute')) {
                ...
                foreach ($reflectionClass->getProperties() as $reflectionProperty) {
                    ...
                    if ($paramMeta = $this->reader->getPropertyAnnotation($reflectionProperty, 'Lv\RpcBundle\Mapping\Param')) {
