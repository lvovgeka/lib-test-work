...
use Exception;
use Lv\RpcBundle\Server\Method;
use Lv\RpcBundle\Server\Mapper;
use Lv\RpcBundle\Server\Request;
use Lv\RpcBundle\Server\Response;
use Lv\RpcBundle\Server\Exceptions\RpcException;
use Lv\RpcBundle\Server\Exceptions\ParseErrorException;
use Lv\RpcBundle\Server\Exceptions\InternalErrorException;
use Lv\RpcBundle\Server\Exceptions\InvalidParamsException;
use Lv\RpcBundle\Server\Exceptions\InvalidRequestException;
use Lv\RpcBundle\Server\Exceptions\MethodNotFoundException;
use Lv\RpcBundle\Server\Exceptions\MethodNotGrantedException;
...
class Handler
{
     ...
     * Rpc mapper
     *
     * @var \Lv\RpcBundle\Server\Mapper|null
     */
    protected $mapper;
    ...
    public function handleHttpRequest(HttpRequest $httpRequest)
    {
        ...
        try {
            $payload = $this->parsePayload($httpRequest->getContent());
     ...
     * @return mixed
     *
     * @throws \Lv\RpcBundle\Server\Exceptions\ParseErrorException
     */
     ...
     * @param  array $payload
     *
     * @return \Lv\RpcBundle\Server\Response|null
     */
    public function handleFromPayload($payload)
    {
        try {
            return $this->handleRequest(Request::createFromPayload($payload));
        } catch (RpcException $exception) {
     ...
     * Handle request.
     *
     * @param  \Lv\RpcBundle\Server\Request $request
     *
     * @return \Lv\RpcBundle\Server\Response|null
     */
